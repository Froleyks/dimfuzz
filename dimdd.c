...
/* Copyright (c) 2025, Nils Froleyks, Johannes Kepler University. */
...
NEXT:
  ...
  if (qbf)
    msg("this is actually a QBF instance");
  else
    msg("this is a propositional instance");
}

static int maskstatus(int status) {
  int res = status;
  if (masksignals)
    res = WEXITSTATUS(res);
  return res;
...
static int clausesatisfied(int i) {
  int j, lit;
  if (!clauses[i])
    return 1;
  j = 0;
...
static void setup(int compute_expected) {
...
static void reduce(void) {
  ...
  while (width) {
    ...
    msg("reduce(%d) width %d removed %d clauses", round, width, removed);
...
static void shrink(void) {
  ...
  for (i = 0; i < size_clauses; i++) {
    ...
    if (isatty(2)) {
      fprintf(stderr, "[cnfdd] shrink(%d) removed %d completed %d/%d\r", round,
              removed, i, size_clauses);
...
static void opts(void) {
  ...
  if (removeopts) {
    ...
    for (i = 0; i < nopts; i++) {
      ...
      if (isatty(2)) {
        fprintf(stderr, "[cnfdd] opts(%d) removed %d completed %d/%d\r", round,
                removed, c, n);
  ...
  if (reduced)
    msg("opts(%d) reduced %d option values in %d reductions", round, reduced,
        reductions);
}
